...
pub unsafe extern "win64" fn sys_execute_app(path_ptr: *const u8) {
    unsafe {
        // ... (vorheriger Code bleibt gleich bis zum App-Start) ...
        ...
        // NEU: Stack für die App setzen
        let app_stack_top = LOAD_ADDR + (PAGES * 4096) - 16;
        ...
        // NEU: App mit korrektem Stack aufrufen
        asm!(
            "mov rsp, {0}",
            "call {1}",
            // Nach App-Rückkehr: Stack wiederherstellen
            "mov rsp, rbp",
            in(reg) app_stack,
            in(reg) entry_point,
            in("rdi") syscalls_ptr,
...
unsafe fn draw_signal(color: u32) {
    unsafe {
        if !FB_PTR.is_null() {
            for i in 0..50_000isize {
                *FB_PTR.offset(i) = color;
            }
        }
    }
}
...
fn main(_handle: Handle, mut system_table: SystemTable<Boot>) -> Status {
    ...
    draw_signal(0x0000FF); // Blau = zurück im Manager
    return;
    }
}