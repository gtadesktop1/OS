...
use uefi::proto::media::file::{
    File, FileAttribute, FileMode, Directory,
};
...
const LOAD_ADDR: usize = 0x1000000;
...
// Suche in deiner main.rs den sys_get_key Syscall und ändere ihn so:

static mut APP_KILLED: bool = false;

// Ändere sys_get_key im Kernel:
#[unsafe(no_mangle)]
pub extern "win64" fn sys_get_key() -> u16 {
    unsafe {
        ...
        // Wir schauen nach, ob eine Taste gedrückt wurde
        ...
        if key == 0xFF0E { // F4 gedrückt!
            APP_KILLED = true;
            return 0; 
        }
        
        if APP_KILLED { return 0; } // Wenn gekillt, keine Tasten mehr an App
        key
    }
}
...
pub unsafe extern "win64" fn sys_list_files(
    ...
    unsafe {
        ...
        let handles = match bs.find_handles::<SimpleFileSystem>() { Ok(h) => h, Err(_) => return 0 };
        for &h in &handles {
            let count = {
                let Ok(mut fs) = bs.open_protocol_exclusive::<SimpleFileSystem>(h) else { continue };
                let Ok(mut root) = fs.open_volume() else { continue };
                let mut dir: Directory = if path_str.is_empty() { root } else {
                    let mut buf = [0u16; 128];
                    let Ok(cpath) = CStr16::from_str_with_buf(path_str, &mut buf) else { continue };
                    match root.open(cpath, FileMode::Read, FileAttribute::DIRECTORY) {
                        Ok(fh) => match fh.into_directory() { Some(d) => d, None => continue },
                        Err(_) => continue,
                    }
                };
                let mut count = 0u32;
...
pub unsafe extern "win64" fn sys_file_write(path_ptr: *const u8, data_ptr: *const u8, size: usize) -> i32 {
    unsafe {
        ...
        let bs = &*BOOT_SERVICES_PTR;
        let path_str = core::ffi::CStr::from_ptr(path_ptr as *const i8).to_str().unwrap_or("");
        let handles = match bs.find_handles::<SimpleFileSystem>() { Ok(h) => h, Err(_) => return -1 };
        for &h in &handles {
            let result = {
                let Ok(mut fs) = bs.open_protocol_exclusive::<SimpleFileSystem>(h) else { continue };
                let Ok(mut root) = fs.open_volume() else { continue };
...
pub unsafe extern "win64" fn sys_execute_app(path_ptr: *const u8) {
    unsafe {
        // Um Schachtelung zu erlauben, nutzen wir einen eigenen Speicherbereich pro App-Tiefe
        // oder laden es sauber in den reservierten LOAD_ADDR Bereich.
        let ptr = LOAD_ADDR as *mut u8;
        
        // Temporärer Buffer zum Laden, um den aktuell laufenden Manager nicht zu korrumpieren
        let mut temp_buf = vec![0u8; PAGES * 4096];
        let bytes_read = sys_file_read(path_ptr, temp_buf.as_mut_ptr(), temp_buf.len());
        
        if bytes_read <= 0 { return; }

        // Kopiere in den Executable-Bereich
        core::ptr::copy_nonoverlapping(temp_buf.as_ptr(), ptr, bytes_read as usize);
        ...
        let entry_point: *mut u8 =
            if (ptr as *const u16).read_unaligned() == 0x5A4D {
                ...
                if (ptr.offset(pe_off) as *const u32).read_unaligned() == 0x0000_4550 {
                    let rva = (ptr.offset(pe_off + 0x28) as *const u32).read_unaligned() as isize;
                    ptr.offset(rva)
                } else { ptr }
            } else { ptr };

        let app_stack_top = LOAD_ADDR + (PAGES * 4096) - 32;
        let app_stack = app_stack_top as *mut u8;
        ...
        // WICHTIG: Stack-Sicherung für Rückkehr
        asm!(
            "push rbp",
            "mov rbp, rsp",
            "mov rsp, {0}",
            "sub rsp, 32",          // Shadow Space für Win64 ABI
            "call {1}",
            "mov rsp, rbp",
            "pop rbp",
            in(reg) app_stack,
...
fn main(_handle: Handle, mut system_table: SystemTable<Boot>) -> Status {
    ...
    unsafe {
        ...
        RT_PTR = system_table.runtime_services() as *const uefi::table::runtime::RuntimeServices;
    }
    ...
    let _ = bt.allocate_pages(AllocateType::Address(LOAD_ADDR as u64), MemoryType::LOADER_DATA, PAGES);
